...
float SoundEngine::_getVolumeMult( double x, double y, double z )
{
	const double dx = x - _x;
	const double dy = y - _y;
	const double dz = z - _z;
	const double dist = Mth::sqrt(dx*dx + dy*dy + dz*dz);
	const float out = (float)Mth::clamp(1.1 - dist * _invMaxDistance, -1.0, 1.0);
	return out;
}

#if defined(PRE_ANDROID23)
void SoundEngine::play(const std::string& name, double x, double y, double z, float volume, float pitch) {

...
void ScrollingPane::tick() {
	...
	if (isNotSet(SF_HardLimits) && !Mouse::isButtonDown(MouseAction::ACTION_LEFT) && !dragging && !tracking && !decelerating) {
		...
		if (targetX > 0.0f) {
			...
			targetX = Mth::lerp(targetX, (float)minPoint.x, kWheelOverscrollRestoreAlpha);
			corrected = true;
		}
		...
		if (targetY > 0.0f) {
			...
			targetY = Mth::lerp(targetY, (float)minPoint.y, kWheelOverscrollRestoreAlpha);
			corrected = true;
		}
...
void ScrollingPane::touchesMoved(float x, float y, int t)
{
	...
	if (this->dragging) {
		...
		if (isNotSet(SF_HardLimits)) {
			...
			f = Mth::Min(Mth::Max((float)this->minPoint.x, f), 0.0f);
			a = Mth::Min(Mth::Max((float)this->minPoint.y, a), 0.0f);
		}
		if (this->firstDrag) {
...
void ScrollingPane::startDecelerationAnimation( bool force )
{
	...
	if (this->pagingEnabled) {
		...
		this->maxDecelerationPoint.x = Mth::Min(0.0f, (float)std::ceil(this->_contentOffsetBeforeDeceleration.x / this->size.w) * this->size.w);
		this->maxDecelerationPoint.y = Mth::Min(0.0f, (float)std::ceil(this->_contentOffsetBeforeDeceleration.y / this->size.h) * this->size.h);
	}
...
void ScrollingPane::stepThroughDecelerationAnimation(bool noAnimation) {
	...
	if (isSet(SF_HardLimits)) {
		float a = Mth::Min(Mth::Max((float)this->minPoint.x, g), 0.0f);
		if (a != g) {
			g = a;
			this->decelerationVelocity.x = 0;
		}
		float c = Mth::Min(Mth::Max((float)this->minPoint.y, h), 0.0f);
		if (c != h) {
			h = c;
...
void ScrollingPane::scrollBy(float dx, float dy) {
	...
	if (isSet(SF_HardLimits)) {
		targetX = Mth::clamp(targetX, (float)minPoint.x, 0.0f);
		targetY = Mth::clamp(targetY, (float)minPoint.y, 0.0f);
	} else {
		if (targetX > 0.0f) {
...
void ScrollingPane::snapContentOffsetToBounds(bool a) {
	...
	if (this->pagingEnabled) {
		...
		if (isNotSet(SF_HardLimits)) {
			c.x = Mth::Min((float)Mth::Max(this->minPoint.x, this->_contentOffset.x), 0.0f);
			c.y = Mth::Min((float)Mth::Max(this->minPoint.y, this->_contentOffset.y), 0.0f);
...
void ScrollingPane::updateHorizontalScrollIndicator()
{
	...
	if (this->_contentOffset.x > 0) {
		d = std::floor(0.5f + Mth::Max(d - (float)this->_contentOffset.x, PKScrollIndicatorThickness));
		a = 1;
	} else {
		if (this->_contentOffset.x < -(this->adjustedContentSize.w - this->size.w)) {
			d = std::floor(0.5f + Mth::Max(d + (float)this->adjustedContentSize.w - this->size.w + (float)this->contentOffset().x, PKScrollIndicatorThickness));
			a = this->size.w - d - c;
		}
	}
...
void ScrollingPane::updateVerticalScrollIndicator()
{
	...
	if (this->_contentOffset.y > 0) {
		d = std::floor(0.5f + Mth::Max(d - (float)this->_contentOffset.y, PKScrollIndicatorThickness));
		b = 1;
	} else {
		if (this->_contentOffset.y < -(this->adjustedContentSize.h - this->size.h)) {
			d = std::floor(0.5f + Mth::Max(d + (float)this->adjustedContentSize.h - this->size.h + (float)this->contentOffset().y, PKScrollIndicatorThickness));
			b = this->size.h - d - c;
		}
	}
	//this->vScroll.x = a;

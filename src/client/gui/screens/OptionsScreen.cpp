...
void OptionsScreen::setupPositions() {
	...
	btnClose->x = (minecraft->SafeZone.right * GUI::InvGuiScale) - btnClose->width;
	btnClose->y = 0;
	...
	for (std::vector<Touch::TButton*>::iterator it = categoryButtons.begin(); it != categoryButtons.end(); ++it) {
		...
		(*it)->x = minecraft->SafeZone.left * GUI::InvGuiScale;
	...
	if (btnCredits != NULL) {
		btnCredits->x = (minecraft->SafeZone.right * GUI::InvGuiScale) - btnCredits->width;
	...
	for (std::vector<OptionsGroup*>::iterator it = optionPanes.begin(); it != optionPanes.end(); ++it) {
		...
		if (categoryButtons.size() > 0 && categoryButtons[0] != NULL) {
			...
			(*it)->x = (minecraft->SafeZone.left * GUI::InvGuiScale) + categoryButtons[0]->width;
			(*it)->y = bHeader->height;
			(*it)->width = width - (*it)->x;

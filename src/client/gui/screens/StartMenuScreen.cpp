...
void StartMenuScreen::render( int xm, int ym, float a )
{
	...
	drawString(font, version, (int) (minecraft->SafeZone.right * minecraft->gui.InvGuiScale) - font->width(version) - 2, height - 10, 0xffcccccc);//0x666666);
	drawString(font, copyright, (int) (minecraft->SafeZone.left * minecraft->gui.InvGuiScale) + 2, height - 20, 0xffffff);
	glEnable2(GL_BLEND);
		...
		blit((int) (minecraft->SafeZone.left * minecraft->gui.InvGuiScale) + 2, height - 10, 0, 0, 8, 8, 256, 256);
	{
			...
			Gui::drawColoredString(font, txt, (int) (minecraft->SafeZone.left * minecraft->gui.InvGuiScale) + 12, height - 10, 255);
			// underline link

	...
	// Render player cape if available
{
    ...
    if (player) {
        ...
        if (!capeTex.empty()) {
            ...
            float capeX = player->getCapePrevX() + (player->getCapeX() - player->getCapePrevX()) * pt;
            float capeY = player->getCapePrevY() + (player->getCapeY() - player->getCapePrevY()) * pt;
            float capeZ = player->getCapePrevZ() + (player->getCapeZ() - player->getCapePrevZ()) * pt;

            float px = player->xo + (player->x - player->xo) * pt;
            float py = player->yo + (player->y - player->yo) * pt;
            float pz = player->zo + (player->z - player->zo) * pt;
            ...
            float sinYaw = Mth::sin(rad);
            float cosYaw = -Mth::cos(rad);
...
void HumanoidMobRenderer::render( Entity* mob_, double x, double y, double z, float rot, float a ) {
	Mob* mob = (Mob*)mob_;

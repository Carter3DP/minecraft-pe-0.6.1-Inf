	...
	t(Tesselator::instance)
{
	...
	radius = Mth::sqrt((double)(xs * xs + ys * ys + zs * zs)) * 0.5;
...
{
	glTranslatef2((double)x, (double)y, (double)z);
}
...
{
	double xd = player->x - xm;
	double yd = player->y - ym;
	double zd = player->z - zm;
	return xd * xd + yd * yd + zd * zd;
}

double Chunk::squishedDistanceToSqr( const Entity* player ) const
{
	double xd = player->x - xm;
	double yd = (player->y - ym) * 2;
	double zd = player->z - zm;
	return xd * xd + yd * yd + zd * zd;
}

		...
		do {
			...
			for (int i = from; i < to; i++) {
				...
				if (sortedChunks[i]->visible && !sortedChunks[i]->occlusion_querying) {
					double dist = Mth::sqrt(sortedChunks[i]->distanceToSqr(player));

					int frequency = (int) (1 + dist / 128);

					if (ticks % frequency == i % frequency) {
						Chunk* chunk = sortedChunks[i];
						double xt = chunk->x - xOff;
						double yt = chunk->y - yOff;
						double zt = chunk->z - zOff;
						double xdd = xt - xo;
						double ydd = yt - yo;
						double zdd = zt - zo;
...
void LevelRenderer::renderClouds( double alpha ) {
	...
	double yOffs = mc->player->yOld + (mc->player->y - mc->player->yOld) * alpha;
	int s = 32;
	int d = 256 / s;
	Tesselator& t = Tesselator::instance;
	...
	double scale = 1 / 2048.0;

	double time = (ticks + alpha);
	double xo = mc->player->xo + (mc->player->x - mc->player->xo) * alpha + time * 0.03;
	...
	for (int xx = -s * d; xx < +s * d; xx += s) {
		for (int zz = -s * d; zz < +s * d; zz += s) {
			t.vertexUV(xx, yy, zz + s, xx * scale + uo, (zz + s) * scale + vo);
			t.vertexUV(xx + s, yy, zz + s, (xx + s) * scale + uo, (zz + s) * scale + vo);
			t.vertexUV(xx + s, yy, zz, (xx + s) * scale + uo, zz * scale + vo);
			t.vertexUV(xx, yy, zz, xx * scale + uo, zz * scale + vo);
		}
	}
...
void LevelRenderer::renderAdvancedClouds(float alpha) {
	...
	double px = mc->player->xOld + (mc->player->x - mc->player->xOld) * alpha;
	double py = mc->player->yOld + (mc->player->y - mc->player->yOld) * alpha;
	double pz = mc->player->zOld + (mc->player->z - mc->player->zOld) * alpha;
	...
	double xOffs = xo - Mth::floor(xo);
	double zOffs = zo - Mth::floor(zo);
...
void LevelRenderer::playSound(const std::string& name, double x, double y, double z, float volume, float pitch) {
...
void LevelRenderer::addParticle(const std::string& name, double x, double y, double z, double xa, double ya, double za, int data) {
...
void LevelRenderer::addParticle(ParticleType::Id name, double x, double y, double z, double xa, double ya, double za, int data) {
	switch (name) {
	case ParticleType::bubble: addParticle("bubble", x, y, z, xa, ya, za, data); break;
	case ParticleType::crit: addParticle("crit", x, y, z, xa, ya, za, data); break;
	case ParticleType::smoke: addParticle("smoke", x, y, z, xa, ya, za, data); break;
	case ParticleType::explode: addParticle("explode", x, y, z, xa, ya, za, data); break;
	case ParticleType::flame: addParticle("flame", x, y, z, xa, ya, za, data); break;
	case ParticleType::lava: addParticle("lava", x, y, z, xa, ya, za, data); break;
	case ParticleType::largesmoke: addParticle("largesmoke", x, y, z, xa, ya, za, data); break;
	case ParticleType::reddust: addParticle("reddust", x, y, z, xa, ya, za, data); break;
	case ParticleType::iconcrack: addParticle("iconcrack", x, y, z, xa, ya, za, data); break;
	case ParticleType::snowballpoof: addParticle("snowballpoof", x, y, z, xa, ya, za, data); break;
	case ParticleType::largeexplode: addParticle("largeexplode", x, y, z, xa, ya, za, data); break;
	case ParticleType::hugeexplosion: addParticle("hugeexplosion", x, y, z, xa, ya, za, data); break;
	default: break;
	}
}

...
/*private*/
void GameRenderer::setupCamera(double a, int eye) {
	renderDistance = (double) (16 * 16 >> (mc->options.getIntValue(OPTIONS_VIEW_DISTANCE)));
#if defined(ANDROID)
	...
	if (mc->options.getBooleanValue(OPTIONS_BETA_SKY)){
		if (zoom != 1) {
			glTranslatef2((double) zoom_x, (double) -zoom_y, 0);
			glScalef2(zoom, zoom, 1);
			gluPerspective(_setupCameraFov = getFov(a, true), mc->width / (double) mc->height, 0.05, renderDistance * 2.0);
		} else {
			gluPerspective(_setupCameraFov = getFov(a, true), mc->width / (double) mc->height, 0.05, renderDistance * 2.0);
		}
	} else {
		if (zoom != 1) {
			glTranslatef2((double) zoom_x, (double) -zoom_y, 0);
			glScalef2(zoom, zoom, 1);
			gluPerspective(_setupCameraFov = getFov(a, true), mc->width / (double) mc->height, 0.05, renderDistance);
		} else {
			gluPerspective(_setupCameraFov = getFov(a, true), mc->width / (double) mc->height, 0.05, renderDistance);
		}
	}
	...
	glLoadIdentity2();
	if (mc->options.getBooleanValue(OPTIONS_ANAGLYPH_3D)) glTranslatef2((eye * 2 - 1) * 0.10, 0, 0);
...
/*public*/
void GameRenderer::render(double a) {
	TIMER_PUSH("mouse");
	if (mc->player && mc->mouseGrabbed) {
		mc->mouseHandler.poll();
		//printf("Controller.x,y : %f,%f\n", Controller::getX(0), Controller::getY(0));

		double ss = mc->options.getProgressValue(OPTIONS_SENSITIVITY) * 0.6 + 0.2;
		double sens = (ss * ss * ss) * 8;
		double xo = mc->mouseHandler.xd * sens * 4.0;
		double yo = mc->mouseHandler.yd * sens * 4.0;

		const double now = _tick + a;
		double deltaT = now - _lastTickT;
		if (deltaT > 3.0) deltaT = 3.0;
		_lastTickT = now;
		...
		if (_t_keepPic < 0) {
			if (!(mc->screen && !mc->screen->renderGameBehind())) {
				...
				if (!mc->options.getBooleanValue(OPTIONS_HIDEGUI)) {
					...
					mc->gui.render(a, mc->screen != NULL, xMouse, yMouse); //chage Gui::render to take float maybe?
				}
			}}
		TIMER_POP();
	...
	if (mc->player && mc->screen == NULL) {
		if (mc->inputHolder) mc->inputHolder->render(a);  //keep IInputHolder::render as float?
	...
	if (mc->screen != NULL) {
		...
		mc->screen->render(xMouse, yMouse, a);  //same as above
...
/*public*/
void GameRenderer::renderLevel(double a) {
	...
	TIMER_PUSH("pick");
	pick(a);  //uncertain if I should make this function input double or not
	...
	for (int i = 0; i < 2; i++) {
		...
		mc->levelRenderer->cull(&frustum, a);  //why does cull take an a value? it does nothing with it
...
//		if(mc->options.getBooleanValue(OPTIONS_FANCY_GRAPHICS)) {
			...
			glFogf(GL_FOG_START, renderDistance  * 0.6);
...
//		glDisable2(GL_ALPHA_TEST); // vanilla pe disables alpha test here, i renable it so grass side textures dont become opaque - shredder
		...
		mc->textures->loadAndBindTexture("terrain.png");
		// @TODO - below is a commented out float render system that fixes ice transparency, but probs harm performance, add it as an option - shredder
...
double GameRenderer::getFov(double a, bool applyEffects) {
	...
	if (player->health <= 0) {
		double duration = player->deathTime + a;

		fov /= ((1 - 500 / (duration + 500)) * 2.0 + 1);
	}
	return fov + fovOffsetO + (fovOffset - fovOffsetO) * a;
}

/*private*/
void GameRenderer::moveCameraToPlayer(double a) {
	Entity* player = mc->cameraTargetPlayer;

	double heightOffset = player->heightOffset - 1.62;
	...
	if(player->isPlayer() && ((Player*)player)->isSleeping()) {
		...
		if (mc->options.getBooleanValue(OPTIONS_FIXED_CAMERA)) {
			...
			double rotationY = thirdRotationO + (thirdRotation - thirdRotationO) * a;
			double xRot = thirdTiltO + (thirdTilt - thirdTiltO) * a;
			...
			for (int i = 0; i < 8; i++) {
				double xo = (double)((i & 1) * 2 - 1);
				double yo = (double)(((i >> 1) & 1) * 2 - 1);
				double zo = (double)(((i >> 2) & 1) * 2 - 1);
...
/*private*/
void GameRenderer::bobHurt(double a) {
	Mob* player = mc->cameraTargetPlayer;

	double hurt = player->hurtTime - a;
	...
	hurt = (double) Mth::sin(hurt * hurt * hurt * hurt * Mth::PI);

	double rr = player->hurtDir;
...
/*private*/
void GameRenderer::bobView(double a) {
	...
	double wda = player->walkDist - player->walkDistO;
	double b = -(player->walkDist + wda * a);
	double bob = player->oBob + (player->bob - player->oBob) * a;
	double tilt = player->oTilt + (player->tilt - player->oTilt) * a;
	glTranslatef2((float) Mth::sin(b * Mth::PI) * bob * 0.5, -(float) std::abs(Mth::cos(b * Mth::PI) * bob), 0);
	glRotatef2((float) Mth::sin(b * Mth::PI) * bob * 3, 0, 0, 1);
	glRotatef2((float) std::abs(Mth::cos(b * Mth::PI - 0.2) * bob) * 5, 1, 0, 0);
...
bool GameRenderer::updateFreeformPickDirection(float a, Vec3& outDir) {
	...
	if (!glhUnProjectf((float)x, (float)y, 1, lastModelMatrix, lastProjMatrix, vp, pt)) {
		return false;
	}
	Vec3 p1(pt[0] + c.x, pt[1] + c.y, pt[2] + c.z);

	glhUnProjectf((float)x, (float)y, 0, lastModelMatrix, lastProjMatrix, vp, pt);
...
void GameRenderer::prepareAndRenderClouds( LevelRenderer* levelRenderer, double a ) {
	...
	glPushMatrix2();
	glLoadIdentity2();
	gluPerspective(_setupCameraFov = getFov(a, true), mc->width / (double) mc->height, 2, renderDistance * 512);
	glMatrixMode(GL_MODELVIEW);
	glPushMatrix2();
	setupFog(0);
	glDepthMask(false);
	...
	glFogf(GL_FOG_START, renderDistance * 4.2 * 0.6);
	glFogf(GL_FOG_END, renderDistance * 4.2);
	levelRenderer->renderClouds(a);
	glFogf(GL_FOG_START, renderDistance  * 0.6);
	glFogf(GL_FOG_END, renderDistance);
	glDisable2(GL_FOG);

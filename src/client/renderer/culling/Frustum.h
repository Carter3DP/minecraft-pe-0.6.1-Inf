...
public:
    ...
    ///////////////////////////////// NORMALIZE PLANE \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\*
private:
    void normalizePlane(double frustum[16][16], int side)
    {
        // Here we calculate the magnitude of the normal to the plane (point A B C)
        // Remember that (A, B, C) is that same thing as the normal's (X, Y, Z).
        // To calculate magnitude you use the equation:  magnitude = sqrt( x^2 + y^2 + z^2)
        double invMagnitude = Mth::invSqrt(frustum[side][A] * frustum[side][A] + frustum[side][B] * frustum[side][B] + frustum[side][C] * frustum[side][C]);
    ...
    void calculateFrustum()
    {
        ...
        GLfloat glProj[16];
        GLfloat glModl[16];
        glGetFloatv(GL_PROJECTION_MATRIX, glProj);
        for (int i = 0; i < 16; i++) proj[i] = glProj[i];

        // By passing in GL_MODELVIEW_MATRIX, we can abstract our model view matrix.
        // This also stores it in an array of [16].
        glGetFloatv(GL_MODELVIEW_MATRIX, glModl);
        for (int i = 0; i < 16; i++) modl[i] = glModl[i];

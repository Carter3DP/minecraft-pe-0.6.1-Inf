...
void Throwable::shoot( double xd, double yd, double zd, float pow, float uncertainty ) {
	double dist = Mth::sqrt(xd * xd + yd * yd + zd * zd);
	...
	float sd = (float) Mth::sqrt(xd * xd + zd * zd);
        ...
        yRotO = this->yRot = (float) (std::atan2(xd, zd) * 180 / Mth::PI);
        xRotO = this->xRot = (float) (std::atan2(yd, sd) * 180 / Mth::PI);
	life = 0;
}
...
void Throwable::tick() {
	...
	yRot = std::atan2(xd, zd) * 180 / Mth::PI;
	xRot = std::atan2(yd, sd) * 180 / Mth::PI;

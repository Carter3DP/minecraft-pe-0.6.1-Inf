...
private:
    ...
    bool canMoveDirectly(const Vec3& startPos, const Vec3& stopPos, int sx, int sy, int sz) {
        ...
        double nf = 1.0 / Mth::sqrt(distSqr);
        dirX *= nf;
        dirZ *= nf;
        ...
        double deltaX = 1 / Mth::abs(dirX);
        double deltaZ = 1 / Mth::abs(dirZ);
    ...
    bool canWalkOn(int x, int y, int z, int sx, int sy, int sz, const Vec3& startPos, double goalDirX, double goalDirZ) {
        int startX = x - sx / 2;
        int startZ = z - sz / 2;
    ...
    bool canWalkAbove(int startX, int startY, int startZ, int sx, int sy, int sz, const Vec3& startPos, double goalDirX, double goalDirZ) {
        for (int xx = startX; xx < startX + sx; xx++) {

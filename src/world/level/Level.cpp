...
/*public*/
Player* Level::getNearestPlayer(double x, double y, double z, float maxDist) {
...
/*public*/
void Level::tick() {
	MCPE_CRASH_TRACE("[IW] Level::tick begin time=%ld entities=%d players=%d chunks=%s\n",
		levelData.getTime(), (int)entities.size(), (int)players.size(), gatherChunkSourceStats().c_str());
	if (!isClientSide && levelData.getSpawnMobs()) {
		...
		if (++_mobSpawnTick >= MobSpawnInterval) {
			...
			MCPE_CRASH_TRACE("[IW] Level::tick mobSpawner begin enemies=%d friendlies=%d\n",
				_spawnEnemies && difficulty > Difficulty::PEACEFUL,
				_spawnFriendlies && (levelData.getTime() % 400) < MobSpawnInterval);
			MobSpawner::tick(this,	_spawnEnemies && difficulty > Difficulty::PEACEFUL,
				_spawnFriendlies && (levelData.getTime() % 400) < MobSpawnInterval);
			MCPE_CRASH_TRACE("[IW] Level::tick mobSpawner end\n");
			TIMER_POP();
		}
	}

	TIMER_PUSH("chunkSource");
	MCPE_CRASH_TRACE("[IW] Level::tick chunkSource begin\n");
	_chunkSource->tick();
	MCPE_CRASH_TRACE("[IW] Level::tick chunkSource end\n");
	...
	MCPE_CRASH_TRACE("[IW] Level::tick pending begin count=%d\n", (int)_tickNextTickSet.size());
	tickPendingTicks(false);
	MCPE_CRASH_TRACE("[IW] Level::tick pending end count=%d\n", (int)_tickNextTickSet.size());

	TIMER_POP_PUSH("tickTiles");
	MCPE_CRASH_TRACE("[IW] Level::tick tiles begin\n");
	tickTiles();
	MCPE_CRASH_TRACE("[IW] Level::tick tiles end\n");

	TIMER_POP_PUSH("sendEntityData");
	MCPE_CRASH_TRACE("[IW] Level::tick entityData begin\n");
	...
	MCPE_CRASH_TRACE("[IW] Level::tick entityData end\n");
	...
	//}
	TIMER_POP();
	MCPE_CRASH_TRACE("[IW] Level::tick end\n");
}
...
void Level::tickTiles() {
	...
	for (ChunkPosSet::iterator it = _chunksToPoll.begin(); it != _chunksToPoll.end(); ++it) {
		...
		if (!lc) {
			MCPE_CRASH_TRACE("[IW] Level::tickTiles NULL chunk (%d,%d)\n", cp.x, cp.z);
			TIMER_POP();
			continue;
		}
		...
		for (int i = 0; i < 20; i++) { //@todo: CHUNK_TILE_TICK_COUNT
			...
			int id = lc->getTile(x, y, z);
			if (id < 0 || id >= Tile::NUM_BLOCK_TYPES || Tile::tiles[id] == NULL) {
				if (id != 0)
					MCPE_CRASH_TRACE("[IW] Level::tickTiles invalid tile id=%d world=(%d,%d,%d) chunk=(%d,%d)\n",
						id, x + xo, y, z + zo, cp.x, cp.z);
				continue;
			}
...
bool Level::tickPendingTicks(bool force) {
	...
	for (int i = 0; i < count; i++) {
		...
		if (hasChunksAt(td->x - r, td->y - r, td->z - r, td->x + r, td->y + r, td->z + r)) {
			...
			if (id == td->tileId && id > 0 && id < Tile::NUM_BLOCK_TYPES && Tile::tiles[id] != NULL) {
				Tile::tiles[id]->tick(this, td->x, td->y, td->z, &random);
			} else if (id == td->tileId && id > 0) {
				MCPE_CRASH_TRACE("[IW] Level::tickPendingTicks invalid tile id=%d at (%d,%d,%d)\n",
					id, td->x, td->y, td->z);
			}
		}
...
const Material* Level::getMaterial(int x, int y, int z) {
	...
	if (t < 0 || t >= Tile::NUM_BLOCK_TYPES || Tile::tiles[t] == NULL) {
		MCPE_CRASH_TRACE("[IW] Level::getMaterial invalid tile id=%d at (%d,%d,%d)\n", t, x, y, z);
		return Material::air;
	}
...
void Level::playSound(double x, double y, double z, const std::string& name, float volume, float pitch) {
	for (unsigned int i = 0; i < _listeners.size(); i++) {
...
void Level::addParticle(const std::string& id, double x, double y, double z, double xd, double yd, double zd, int data /* = 0 */) {
	for (unsigned int i = 0; i < _listeners.size(); i++)
		_listeners[i]->addParticle(id, x, y, z, xd, yd, zd, data);
}

void Level::addParticle(ParticleType::Id id, double x, double y, double z, double xd, double yd, double zd, int data /* = 0 */) {
	for (unsigned int i = 0; i < _listeners.size(); i++)
		_listeners[i]->addParticle(id, x, y, z, xd, yd, zd, data);
}
...
class DistanceEntitySorter {
	Vec3 c;
public:
	DistanceEntitySorter(double x, double y, double z) : c(x, y, z) {}
	bool operator() (const Entity* c0, const Entity* c1) {
		const double d0 = c.distanceToSqr(c0->x, c0->y, c0->z);
		const double d1 = c.distanceToSqr(c1->x, c1->y, c1->z);
		return d0 < d1;
	}
};
...
void Level::explode(Entity* source, double x, double y, double z, float r) {
	explode(source, x, y, z, r, false);
}

/* in java, this returns an Explosion */
void Level::explode(Entity* source, double x, double y, double z, float r, bool fire) {
	if (!isClientSide) {
...
bool Level::isSolidBlockingTile(int x, int y, int z)
{
	...
	if (tileId < 0 || tileId >= Tile::NUM_BLOCK_TYPES) {
		MCPE_CRASH_TRACE("[IW] Level::isSolidBlockingTile invalid tile id=%d at (%d,%d,%d)\n", tileId, x, y, z);
		return false;
	}
...
bool Level::isSolidRenderTile(int x, int y, int z) {
	int tileId = getTile(x, y, z);
	if (tileId < 0 || tileId >= Tile::NUM_BLOCK_TYPES) {
		MCPE_CRASH_TRACE("[IW] Level::isSolidRenderTile invalid tile id=%d at (%d,%d,%d)\n", tileId, x, y, z);
		return false;
	}
	Tile* tile = Tile::tiles[tileId];
	if (tile == NULL) return false;

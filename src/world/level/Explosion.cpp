...
Explosion::Explosion(Level* level, Entity* source, double x, double y, double z, float r)
:	level(level),
	source(source),
	x(x),
	y(y),
		...
		for (int yy = 0; yy < size; yy++)
			...
			for (unsigned int i = 0; i < entities.size(); i++) {
				...
				if (dist <= 1) {
					...
					double ida = 1.0 / Mth::sqrt(xa * xa + ya * ya + za * za);
...
void Explosion::finalizeExplosion()
{
	...
	for (TilePosSet::const_iterator cit = toBlow.begin(); cit != toBlow.end(); ++cit, ++j) {
		...
		do {
			...
			double invdd = 1.0 / Mth::sqrt(xd * xd + yd * yd + zd * zd);

...
{
	delete regionFile;
	for (RegionFileMap::iterator it = regionFiles.begin(); it != regionFiles.end(); ++it)
		delete it->second;
	delete loadedLevelData;
}

int ExternalFileLevelStorage::floorDiv32(int value)
{
	return value >= 0 ? value / 32 : ((value + 1) / 32) - 1;
}

int ExternalFileLevelStorage::localChunkCoord(int value)
{
	int region = floorDiv32(value);
	return value - region * 32;
}

RegionFile* ExternalFileLevelStorage::getLegacyRegionFile()
{
	if (!regionFile)
	{
		MCPE_CRASH_TRACE("[IW] ExternalFileLevelStorage opening legacy chunks.dat\n");
		regionFile = new RegionFile(levelPath);
		if (!regionFile->open())
		{
			MCPE_CRASH_TRACE("[IW] ExternalFileLevelStorage failed to open legacy chunks.dat\n");
			delete regionFile;
			regionFile = NULL;
			return NULL;
		}
...
RegionFile* ExternalFileLevelStorage::getRegionFile(int chunkX, int chunkZ, bool create)
{
	int regionX = floorDiv32(chunkX);
	int regionZ = floorDiv32(chunkZ);

	char filename[128];
	sprintf(filename, "region/r.%d.%d.dat", regionX, regionZ);
	std::string key(filename);

	RegionFileMap::iterator found = regionFiles.find(key);
	if (found != regionFiles.end())
		return found->second;

	std::string regionPath = levelPath + "/region";
	if (!create && !exists((levelPath + "/" + key).c_str()))
	{
		MCPE_CRASH_TRACE("[IW] ExternalFileLevelStorage region missing chunk=(%d,%d) region=(%d,%d) file=%s\n",
			chunkX, chunkZ, regionX, regionZ, key.c_str());
		return NULL;
	}

	if (!createFolderIfNotExists(regionPath.c_str()))
	{
		MCPE_CRASH_TRACE("[IW] ExternalFileLevelStorage failed to create region dir %s\n", regionPath.c_str());
		return NULL;
	}

	RegionFile* file = new RegionFile(levelPath, key);
	if (!file->open())
	{
		MCPE_CRASH_TRACE("[IW] ExternalFileLevelStorage failed to open region chunk=(%d,%d) region=(%d,%d) file=%s\n",
			chunkX, chunkZ, regionX, regionZ, key.c_str());
		delete file;
		return NULL;
	}

	MCPE_CRASH_TRACE("[IW] ExternalFileLevelStorage opened region chunk=(%d,%d) region=(%d,%d) file=%s\n",
		chunkX, chunkZ, regionX, regionZ, key.c_str());
	regionFiles[key] = file;
	return file;
}
...
{
	MCPE_CRASH_TRACE("[IW] ExternalFileLevelStorage::save begin chunk=(%d,%d)\n", levelChunk->x, levelChunk->z);
	RegionFile* file = getRegionFile(levelChunk->x, levelChunk->z, true);
	if (!file)
	{
		MCPE_CRASH_TRACE("[IW] ExternalFileLevelStorage::save no region chunk=(%d,%d)\n", levelChunk->x, levelChunk->z);
		return;
	}
	...
	bool wrote = file->writeChunk(localChunkCoord(levelChunk->x), localChunkCoord(levelChunk->z), chunkData);
	MCPE_CRASH_TRACE("[IW] ExternalFileLevelStorage::save end chunk=(%d,%d) local=(%d,%d) bytes=%d wrote=%d\n",
		levelChunk->x, levelChunk->z, localChunkCoord(levelChunk->x), localChunkCoord(levelChunk->z),
		chunkData.GetNumberOfBytesUsed(), wrote);
...
{
	MCPE_CRASH_TRACE("[IW] ExternalFileLevelStorage::load begin chunk=(%d,%d)\n", x, z);
	RakNet::BitStream* chunkData = NULL;
	RegionFile* file = getRegionFile(x, z, false);
	if (!file || !file->readChunk(localChunkCoord(x), localChunkCoord(z), &chunkData))
	{
		MCPE_CRASH_TRACE("[IW] ExternalFileLevelStorage::load region miss chunk=(%d,%d) local=(%d,%d)\n",
			x, z, localChunkCoord(x), localChunkCoord(z));
		if (x < 0 || z < 0 || x >= 32 || z >= 32)
		{
			MCPE_CRASH_TRACE("[IW] ExternalFileLevelStorage::load outside legacy range chunk=(%d,%d)\n", x, z);
			return NULL;
		}

		RegionFile* legacyFile = getLegacyRegionFile();
		if (!legacyFile || !legacyFile->readChunk(x, z, &chunkData))
		{
			//LOGI("Failed to read data for %d, %d\n", x, z);
			MCPE_CRASH_TRACE("[IW] ExternalFileLevelStorage::load legacy miss chunk=(%d,%d)\n", x, z);
			return NULL;
		}
		MCPE_CRASH_TRACE("[IW] ExternalFileLevelStorage::load legacy hit chunk=(%d,%d)\n", x, z);
	}
	else
	{
		MCPE_CRASH_TRACE("[IW] ExternalFileLevelStorage::load region hit chunk=(%d,%d) local=(%d,%d)\n",
			x, z, localChunkCoord(x), localChunkCoord(z));
	}
	...
	levelChunk->recalcHeightmap(false);
	levelChunk->unsaved = changed;
	levelChunk->terrainPopulated = true;
	levelChunk->createdFromSave = true;
	MCPE_CRASH_TRACE("[IW] ExternalFileLevelStorage::load end chunk=(%d,%d) changed=%d\n", x, z, changed);

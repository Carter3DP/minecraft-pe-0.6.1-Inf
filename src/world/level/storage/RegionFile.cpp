...
{
	close();
	MCPE_CRASH_TRACE("[IW] RegionFile::open %s\n", filename.c_str());
	...
	else
	{
		...
		if (!file)
		{
			...
			MCPE_CRASH_TRACE("[IW] RegionFile::open failed create %s\n", filename.c_str());
			return false;
		}
...
bool RegionFile::readChunk(int x, int z, RakNet::BitStream** destChunkData)
{
	...
	{
		MCPE_CRASH_TRACE("[IW] RegionFile::readChunk invalid local=(%d,%d) file=%s\n", x, z, filename.c_str());
		return false;
	}
	...
	if (offset == 0)
	{
		...
		MCPE_CRASH_TRACE("[IW] RegionFile::readChunk empty local=(%d,%d) file=%s\n", x, z, filename.c_str());
		return false;
	}

	int sectorNum = offset >> 8;
	int sectorCount = offset & 0xff;
	MCPE_CRASH_TRACE("[IW] RegionFile::readChunk local=(%d,%d) sector=%d count=%d file=%s\n",
		x, z, sectorNum, sectorCount, filename.c_str());

	fseek(file, sectorNum * SECTOR_BYTES, SEEK_SET);
	int length = 0;
	fread(&length, sizeof(int), 1, file);

	if (length <= 0 || length >= (sectorCount * SECTOR_BYTES))
	{
		MCPE_CRASH_TRACE("[IW] RegionFile::readChunk bad length=%d sectorCapacity=%d local=(%d,%d) file=%s\n",
			length, sectorCount * SECTOR_BYTES, x, z, filename.c_str());
		return false;
	}
	length -= sizeof(int);
...
bool RegionFile::writeChunk(int x, int z, RakNet::BitStream& chunkData)
{
	if (x < 0 || x >= SECTOR_COLS || z < 0 || z >= SECTOR_COLS)
	{
		MCPE_CRASH_TRACE("[IW] RegionFile::writeChunk invalid local=(%d,%d) file=%s\n", x, z, filename.c_str());
		return false;
	}
	...
	if (sectorsNeeded > 256)
	{
		...
		MCPE_CRASH_TRACE("[IW] RegionFile::writeChunk too big local=(%d,%d) bytes=%d sectors=%d file=%s\n",
			x, z, size, sectorsNeeded, filename.c_str());
		return false;
	}
	MCPE_CRASH_TRACE("[IW] RegionFile::writeChunk local=(%d,%d) bytes=%d sectors=%d oldSector=%d oldCount=%d file=%s\n",
		x, z, size, sectorsNeeded, sectorNum, sectorCount, filename.c_str());

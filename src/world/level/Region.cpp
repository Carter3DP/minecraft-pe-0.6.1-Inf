...
LevelChunk* Region::getChunkFor(int x, int z) {
	int xc = (x >> 4) - xc1;
	int zc = (z >> 4) - zc1;

	if (xc < 0 || xc >= size_x || zc < 0 || zc >= size_z)
	{
		static int outOfRangeLogs = 0;
		if (outOfRangeLogs < 128)
		{
			MCPE_CRASH_TRACE("[IW] Region::getChunkFor miss block=(%d,%d) chunkOffset=(%d,%d) base=(%d,%d) size=(%d,%d)\n",
				x, z, xc, zc, xc1, zc1, size_x, size_z);
			outOfRangeLogs++;
		}
		return NULL;
	}
...
int Region::getRawBrightness(int x, int y, int z, bool propagate) {
    ...
    LevelChunk* lc = getChunkFor(x, z);
    if (lc == NULL)
        return Level::MAX_BRIGHTNESS - level->skyDarken;

...
/*public*/
void RandomLevelSource::prepareHeights(int xOffs, int zOffs, unsigned char* blocks, /*Biome*/void* biomes, double* temperatures) {
...
void RandomLevelSource::buildSurfaces(int xOffs, int zOffs, unsigned char* blocks, Biome** biomes) {
	...
	double s = 1 / 32.0;
	perlinNoise2.getRegion(sandBuffer, (double)(xOffs * 16), (double)(zOffs * 16), 0, 16, 16, 1, s, s, 1);
	perlinNoise2.getRegion(gravelBuffer, (double)(xOffs * 16), 109.01340, (double)(zOffs * 16), 16, 1, 16, s, 1, s);
	perlinNoise3.getRegion(depthBuffer, (double)(xOffs * 16), (double)(zOffs * 16), 0, 16, 16, 1, s * 2, s * 2, s * 2);

	for (int x = 0; x < 16; x++) {
		for (int z = 0; z < 16; z++) {
			double temp = 1; // @todo: read temp from BiomeSource
			Biome* b = biomes[x + z * 16];
			bool sand = (sandBuffer[x + z * 16] + random.nextFloat() * 0.2) > 0;
			bool gravel = (gravelBuffer[x + z * 16] + random.nextFloat() * 0.2) > 3;
			int runDepth = (int) (depthBuffer[x + z * 16] / 3 + 3 + random.nextFloat() * 0.25);
...
for (int i = 0; i < grassCount; i++) {
	...
	//LOGI("Reading temp: 1\n");
	double* temperatures = level->getBiomeSource()->getTemperatureBlock(NULL, xo + 8, zo + 8, 16, 16);
...
/*private*/
double* RandomLevelSource::getHeights(double* buffer, int x, int y, int z, int xSize, int ySize, int zSize) {
	...
	pnr = perlinNoise1.getRegion(pnr, (double)x, (double)y, (double)z, xSize, ySize, zSize, s / 80.0, hs / 160.0, s / 80.0);
	ar = lperlinNoise1.getRegion(ar, (double)x, (double)y, (double)z, xSize, ySize, zSize, s, hs, s);
	br = lperlinNoise2.getRegion(br, (double)x, (double)y, (double)z, xSize, ySize, zSize, s, hs, s);

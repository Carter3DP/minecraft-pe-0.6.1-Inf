...
std::string CommandServer::parse(ConnectedClient& client, const std::string& s) {
	...
	if (cmd == "player.setTile") {
		...
		e->moveTo(x + 0.5, y, z + 0.5, e->yRot, e->xRot);
		return NullString;
	}
	...
	if (cmd == "player.setPos") {
		...
		double x, y, z;
		if (3 != sscanf(rest.c_str(), "%lf,%lf,%lf", &x, &y, &z)) {
			return Fail;
		}
	...
	if (cmd == "player.getPos") {
		...
		double x = e->x, y = e->y - e->heightOffset, z = e->z;
		apiPosTranslate.to(x, y, z);
		return ToStringOk(x, y, z);
	}
	...
	if (cmd == "entity.setPos") {
		int id;
		double x, y, z;
		if (4 != sscanf(rest.c_str(), "%d,%lf,%lf,%lf", &id, &x, &y, &z)) {
			return Fail;
		}
		Entity* e = mc->level->getEntity(id);
		if (!e) return Fail;

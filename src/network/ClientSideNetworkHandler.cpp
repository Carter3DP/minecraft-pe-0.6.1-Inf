...
{
	if (requestNextChunkIndex < (int)requestNextChunkIndexList.size())
	{
...
{
	return requestNextChunkIndex >= (int)requestNextChunkIndexList.size();
}

void ClientSideNetworkHandler::requestChunksAroundPlayer()
{
	Player* p = minecraft ? minecraft->player : NULL;
	if (!p)
		return;

	bool wasAllLoaded = areAllChunksLoaded();
	int cx = Mth::floor(p->x / (float)CHUNK_WIDTH);
	int cz = Mth::floor(p->z / (float)CHUNK_DEPTH);

	for (int z = cz - RequestChunkRadius; z <= cz + RequestChunkRadius; ++z) {
		for (int x = cx - RequestChunkRadius; x <= cx + RequestChunkRadius; ++x) {
			ChunkPos pos(x, z);
			if (chunksRequested.find(pos) != chunksRequested.end())
				continue;

			IntPair pair;
			pair.x = x;
			pair.y = z;
			requestNextChunkIndexList.push_back(pair);
			chunksRequested.insert(pos);
		}
	}

	if (wasAllLoaded && !areAllChunksLoaded())
		requestNextChunk();
}

bool ClientSideNetworkHandler::isChunkLoaded(int x, int z)
{
	return chunksLoaded.find(ChunkPos(x, z)) != chunksLoaded.end();
...
// 
	...
	chunksLoaded.insert(ChunkPos(packet->x, packet->z));
...
{
	requestNextChunkIndex = 0;
	requestNextChunkIndexList.clear();
	chunksLoaded.clear();
	chunksRequested.clear();
	initialReadySent = false;

	Player* p = minecraft ? minecraft->player : NULL;
	int cx = p ? Mth::floor(p->x / (float)CHUNK_WIDTH) : 0;
	int cz = p ? Mth::floor(p->z / (float)CHUNK_DEPTH) : 0;
	...
	for (int z = cz - RequestChunkRadius; z <= cz + RequestChunkRadius; ++z) {
		for (int x = cx - RequestChunkRadius; x <= cx + RequestChunkRadius; ++x) {
			...
			pair.y = z;
			requestNextChunkIndexList.push_back(pair);
			chunksRequested.insert(ChunkPos(x, z));
		}
	}
}
